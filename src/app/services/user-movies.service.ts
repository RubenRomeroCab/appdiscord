...
export class UserMoviesService {
  ...
  getUserMoviesByUserId(userId: string): Observable<UserMovie[]> {
    const userMoviesCollection = collection(this.firestore, this.collectionName) as CollectionReference<UserMovie>;
    const q = query(userMoviesCollection, where('userId', '==', userId)); // Filtrar por el ID del usuario

    return from(getDocs(q)).pipe(
      map((querySnapshot) =>
        querySnapshot.docs.map((doc) => {
          const data = doc.data() as UserMovie;
          return { ...data, id: doc.id }; // Agregar el ID del documento
        })
      )
    );
  }
